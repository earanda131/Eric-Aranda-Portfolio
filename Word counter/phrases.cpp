...
Phrases::MRmap(const std::map<std::string,std::string> &input,
	...
	for (auto it = input.begin(); it != input.end(); it++ ) {
		...
		do {
			std::string word, second_word, two_word, result;
			iss >> word;
			iss >> second_word;
			two_word = word + " " + second_word;
			// Each word gets assigned a count of 1
			std::remove_copy_if(two_word.begin(), two_word.end(), std::back_inserter(result), std::ptr_fun<int,int>(&std::ispunct));
			out_values.insert(std::pair<std::string,int>(result,1));
		} while (iss);
	}
}
